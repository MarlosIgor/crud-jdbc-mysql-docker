...
public class AnimeRepository {
    ...
    public static List<Anime> findByName(String name) {
        log.info("Encontrar anime pelo nome '{}'", name);
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            log.error("Erro ao tentar encontrar todos os animes", e);
        }
        return animes;
    }
    private static PreparedStatement createPrepareStatementFindByName(Connection conn, String name) throws SQLException {
        String sql = """
                SELECT a.id, a.name, a.episodes, a.producer_id, p.name as 'producer_name'FROM anime_store_code.anime a inner join
                anime_store_code.producer p on a.producer_id = p.id
                where a.name like ?;
                """;
    ...
    public static Optional<Anime> findById(Integer id) {
        log.info("Encontrar anime pelo id '{}'", id);
        try (Connection conn = ConnectionFactory.getConnection();
    ...
    public static void delete(int id) {
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            ps.execute();
            log.info("Deletando anime nas linhas do banco de dados afetadas {}", id);
        } catch (SQLException e) {
            log.error("Erro ao tentar deletar anime '{}'", id, e);
        }
    }
    private static PreparedStatement createPrepareStatementDelete(Connection conn, Integer id) throws SQLException {
        String sql = "DELETE FROM anime_store_code.anime WHERE (id = ?);";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, id);
        return ps;
    }
    ...
    private static PreparedStatement createPrepareStatementUpdate(Connection conn, Anime anime) throws SQLException {
        String sql = "UPDATE anime_store_code.anime SET name = ?, episodes = ? WHERE (id = ?);";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setString(1, anime.getName());
